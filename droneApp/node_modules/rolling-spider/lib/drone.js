    ...
    this.on('flyingStatusChange', function(newStatus) {
      if (newStatus === 0) {
        ...
        if (typeof callback === 'function') {
          ...
          callback();
          console.log('This drone is already on the ground!')
        }
      }
    });
...
function driveBuilder(parameters) {
  ...
  return function(possibleOptions, possibleCallback) {
    ...
    if (this.status.flying) {
      ...
      console.log('Executing command RollingSpider#' + name);
      if (!validSpeed(speed)) {
      ...
      console.log('RollingSpider#' + name + ' when it\'s not in the air isn\'t going to do anything');
